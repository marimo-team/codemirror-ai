 ...
 */
function createInlineDiffSpan(text: string, type: "normal" | "removed" | "added"): HTMLSpanElement {
  const span = document.createElement("span");
...
function createContextualDiffLine(
  ...
  if (isRemoved) {
    ...
    const changedText = fullContext.substring(relativeModifyStart, relativeModifyEnd);
  ...
  toDOM(view: EditorView) {
    ...
    const positionTooltip = () => {
      ...
      if (lineEndPos) {
        ...
        const finalTop = Math.max(8, Math.min(top, window.innerHeight - tooltipRect.height - 8));
    ...
    const scrollHandler = () => {
      requestAnimationFrame(() => {
        ...
        if (!lineEndPos || lineEndPos.top < 0 || lineEndPos.bottom > window.innerHeight) {
