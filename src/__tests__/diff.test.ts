...
function diffTexts(before: string, after: string): string {
  const result = extractDiffOperation({ oldText: before, newText: after }, cursorMarker);
...
class DiffFormatter {
  ...
  private formatModifyOperation(operation: DiffOperationOf<"modify">): string {
    ...
    const oldFormatted = this.formatTextWithIndicators(cleanOldText, oldCursorPos, {
      operationStart: operation.position,
    ...
    const newFormatted = this.formatTextWithIndicators(cleanNewText, newCursorPos, {
      operationStart: operation.position,
  ...
  private formatCursorOperation(_operation: DiffOperationOf<"cursor">): string {
    const cleanNewText = this.newText.replace(this.cursorMarker, "");
    const cursorPos = this.newText.indexOf(this.cursorMarker);
  ...
  private formatTextWithIndicators(
    ...
    for (let i = 0; i < textLines.length; i++) {
      ...
      if (!foundLine && currentPos <= cursorPos && cursorPos <= currentPos + lineLength) {
        ...
        if (options.operationStart >= 0) {
          const opStart = options.operationStart - currentPos + (options.prefix?.length || 0);
          const opEnd = opStart + options.operationLength;

          for (let j = Math.max(0, opStart); j < Math.min(indicator.length, opEnd); j++) {
            // Use the appropriate character for the operation type
            const char = options.addedChar !== " " ? options.addedChar : options.removedChar;
            indicator = indicator.substring(0, j) + char + indicator.substring(j + 1);
          }
        }
